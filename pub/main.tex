...
\begin{abstract}
Ocean models in realistic settings are becoming more common. We consider in this paper methods for their dynamic analysis, particularly calculation of their kinetic energy spatial spectra. Realistic models pose difficulties when classical methods are applied, due to spatial inhomogeneity and complex geometries. Here, we leverage the Helmholz decomposition of a vector field to decompose a 2-D velocity field into its rotational and divergent parts, represented by a streamfunction and velocity potential. The classical problem of Helmholtz decompositions, that the associated boundary conditions are not unique, is sidestepped by arguing appropriate conditions on both cast the kinetic energy of the domain solely in terms of the rotational and divergent contributions. Further exploiting the properties of the Helmholtz equation subject to homogenous boundary conditions allows for complete representation of the streamfunction and potential in terms of orthonormal basis functions. Ultimately this leads to an exact representation of the domain kinetic energy in terms of the projections of the velocity fields on those functions and their eigenvalues, without the need for any windowing or interpolation. This is a decomposition equivalent to Parseval’s equality and permits the interpretation of the results as energy densities in eigenvalue space. Parallels between Fourier analysis and this more general approach argue that the amplitudes plotted against the eigenvalues plays the same role as Fourier based spectra. The generality of the analysis allows it to be applied to open domains, with inflow and outflow, and irregular domains with coastlines and islands. We develop a numerical discretization on an Arakawa C-Grid and a methodology for calculating the eigenfunctions and eigenvalues in complex domains. We apply the technique to some idealized quasi-geostrophic simulations in closed domains and compare the results with traditional fourier spectra to highlight new information obtained. We conclude with a discussion about applications to regional problems with open boundaries and to models on different grids.
\end{abstract}
...
% Substituting these results into \eqref{eq:spectral energy} gives an expression for the kinetic energy
% \begin{equation}
%     K = \frac{1}{2}\sum_{i=0}^\infty \left( \frac{\left( d_i - \oint_{\partial A} (\vec{u} \cdot \hat{n} )\eta_i \hspace{1mm} dl \right)^2}{\sigma_i^2}  + \frac{\left( \zeta_i + \oint_{\partial A} \psi_i (\nabla \xi_i \cdot \hat{n}) \hspace{1mm} dl \right)^2}{\lambda_i^2} \right) \label{eq:spectral energy}
% \end{equation}
...
All of the MQGeometry simulations in this study use identical physical and numerical parameters to the baroclinic double gyre configuration described in \cite{Thiry2022}, which we reiterate here.  
...
\begin{figure}
    \centering
    \includegraphics{figures/c-grid.png}
...
\begin{figure}
    \centering
    \includegraphics{figures/mqgeometry_cornercut/rotational_spectra.png}
    \includegraphics{figures/mqgeometry_dgirreg/rotational_spectra.png}
\end{figure}

% \begin{figure}
%     \centering
%     \includegraphics{figures/square-domain/mqg-vorticity-100y.png}
%     \caption{Relative vorticity for the upper layer after 100 years of simulation spinup}
%     \label{fig:square-domain-vorticity}
% \end{figure}


% \begin{figure}
%     \centering
%     \includegraphics{figures/square-domain/psi_100y_000d.npy.rotational_interior_spectra.png}
%     \caption{Plot of the 2-D spectra. For each eigenvalue, a dot is placed at the x and y wavenumber components and the dot is colored according to the interior rotational energy.}
%     \label{fig:enter-label}
% \end{figure}

% \begin{figure}
%     \centering
%     \includegraphics{figures/square-domain/psi_100y_000d.npy.rotational_isotropic_spectra.png}
%     \caption{Log-log plot of the isotropic spectra for the interior rotational component of the energy. The isotropic spectra is created by selecting only eigenmodes whose x and y components of the spectra are equivalent to within an absolute tolerance of $10^{-13}$.}
%     \label{fig:enter-label}
% \end{figure}
